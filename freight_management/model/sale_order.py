...
class OrderLineFreight(models.Model):
    ...
    # @api.depends('price_un it')
    def _compute_costo_unit(self):
        for rec in self.order_line: 
            rec.costo_unit=rec.price_unit/rec.unidad_bulto
    @api.onchange('order_line')
    def onchange_standard_price(self):
        for rec in self.order_line:
            if rec.price_unit>0:
                self._compute_costo_unit()

        
    
    
    # @api.depends('order_line')
    # def _compute_field_order_line_costo_unit(self):
    #     for record in self:
